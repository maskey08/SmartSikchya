...
from sqlalchemy import Integer, Text, Boolean, DateTime, Float, ForeignKey, func
...
class PracticeSession(Base):
    ...
    start_time:    Mapped[datetime | None] = mapped_column(DateTime(timezone=True), server_default=func.now())
    ...
    responses: Mapped[list["SessionResponse"]] = relationship("SessionResponse", back_populates="session", lazy="select")
...
class SessionResponse(Base):
    ...
    session_id:         Mapped[int | None] = mapped_column(Integer, ForeignKey("practice_sessions.session_id"))
    question_id:        Mapped[int | None] = mapped_column(Integer, ForeignKey("questions.question_id"))
    chapter_id:         Mapped[int | None] = mapped_column(Integer, ForeignKey("chapters.chapter_id"))
